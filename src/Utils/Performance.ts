...
namespace Performance {
	...
	export function getFrameStatsIndex(): number { return frameStatsIndex; }
	export function getFrameStats(index: number): IStringMap<ICallStats> { return frameStats[index]; }
	export function allFrameStats(): Array<IStringMap<ICallStats>>  { return frameStats; }
	export function enableFrameStatLogs(): void { logFrameStats = true; }
	...
	export const privatePropertyTracker = (target: any, key: string): void => {
	...
	export const privateMethodTracker = (target: any, key: string, descriptor: PropertyDescriptor): PropertyDescriptor => {
		const objectName = getObjectName(target, key);
...
export function methodTracker() {
	return Performance.privateMethodTracker;
}

export function propertyTracker() {
	return Performance.privatePropertyTracker;
}
